function PSD_alltime_presentman()

% select file for input
[csv_file, csv_path] = uigetfile({'*.xlsx';'*.csv'}, ...
    'Select file for alltime_presentman input', ...
    "MultiSelect","on");
...
for mi = 1:file_count
    for ei = 2 % event loop* first event start from B
        % read the celltable
        Alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
        for si = 1:4 % 4 sheets
            sheet = si;
            xlrange = strcat(Alphabet(ei),'1:',Alphabet(ei),'94373');
