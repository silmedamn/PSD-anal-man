function f = NOR_CalculateAllPower(data_dir, time_dir, save_dir, result_dir, ...
    sample_name, channel_info, preprocessing, analyze, freq_band_to_analyze)
% Input: 
    ...
    if analyze
        ...
        for to = 1:length(time_order)
            ...
            for i = 1:length(target_times)
                ...
                seg_data_both{i} = voltage_data(start*fs+1:ends*fs);
            end
            ...
            for i = 1:length(target_times)
                start = target_times(i) - 1;
                ends = target_times(i);
                seg_data_pre{i} = voltage_data(start*fs+1:ends*fs);
            end
            ...
            for i = 1:length(target_times)
                ...
                ends = target_times(i) + 1;
                seg_data_post{i} = voltage_data(start*fs+1:ends*fs);
            end
